...
async function resolveOneBotImageFileRef(rawMediaUrl: string): Promise<string> {
  const cleaned = rawMediaUrl.trim();
  if (!cleaned) return "";
  if (/^https?:\/\//i.test(cleaned) || /^base64:\/\//i.test(cleaned)) return cleaned;

  const fromFileUri = cleaned.match(/^file:\/\/(.+)$/i);
  const localPath = fromFileUri ? decodeURIComponent(fromFileUri[1] ?? "") : cleaned;

  try {
    const data = await fs.readFile(localPath);
    return `base64://${data.toString("base64")}`;
  } catch {
    // For bridges that can resolve file URIs, prefer explicit file:// over raw paths.
    if (/^file:\/\//i.test(cleaned)) return cleaned;
    if (localPath.startsWith("/")) return `file://${encodeURI(localPath)}`;
    return cleaned;
  }
}
...
export const onebotPlugin: ChannelPlugin<ResolvedOneBotAccount> = {
  ...
  outbound: {
    ...
    sendMedia: async ({ cfg, to, text, mediaUrl, accountId }) => {
      ...
      const fileRef = cleanedMedia ? await resolveOneBotImageFileRef(cleanedMedia) : "";

      const cqImage = fileRef ? `[CQ:image,file=${fileRef}]` : "";
      const combined = [cleanedText, cqImage].filter(Boolean).join("\n");
      const result = await sendOneBotText({ account, target: to, text: combined || cleanedText });
      return {
        channel: "onebot",
        messageId: result.messageId ?? "",
      };
