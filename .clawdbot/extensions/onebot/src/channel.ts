...
export const onebotPlugin: ChannelPlugin<ResolvedOneBotAccount> = {
  ...
  outbound: {
    ...
    sendMedia: async ({ cfg, to, text, mediaUrl, accountId }) => {
      ...
      // When the assistant attaches local media (e.g. resolved inbound images),
      // most OneBot bridges won't be able to read files from the Clawdbot host path.
      // Base64 is the most portable CQ image encoding across bridges.
      if (fileRef && !/^https?:\/\//i.test(fileRef) && !/^base64:\/\//i.test(fileRef)) {
        try {
          ...
          // Fallback to original ref; some bridges may accept a local path.
